...
# Show civil law sections
print("\n" + "=" * 70)
print("CIVIL LAW SECTIONS LOADED:")
print("=" * 70)
for section in civil_loaded:
    print(f"  ✓ {section.section_number}: {section.text[:60]}...")

# Show special laws
print("\n" + "=" * 70)
print("SPECIAL LAWS LOADED:")
print("=" * 70)
for section in special_loaded:
    print(f"  ✓ {section.section_number}: {section.text[:60]}...")
...
if bns_count == len(bns_loaded):
    print(f"✅ BNS Sections: ALL {bns_count} loaded")
else:
    print(f"❌ BNS Sections: {len(bns_loaded)}/{bns_count} loaded (MISSING {bns_count - len(bns_loaded)})")

if civil_count == len(civil_loaded):
    print(f"✅ Civil Law: ALL {civil_count} loaded")
else:
    print(f"❌ Civil Law: {len(civil_loaded)}/{civil_count} loaded (MISSING {civil_count - len(civil_loaded)})")

if special_count == len(special_loaded):
    print(f"✅ Special Laws: ALL {special_count} loaded")
else:
    print(f"❌ Special Laws: {len(special_loaded)}/{special_count} loaded (MISSING {special_count - len(special_loaded)})")
...
if total_expected == total_loaded:
    ...
    print(f"❌ INCOMPLETE: {total_loaded}/{total_expected} loaded (MISSING {total_expected - total_loaded})")
print(f"{'='*70}")
